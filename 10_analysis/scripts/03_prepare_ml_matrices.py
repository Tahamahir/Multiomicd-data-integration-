...
# ============================================================
# PHASE 2 - PREPARE ML MATRICES
# ------------------------------------------------------------
# Ce script :
# 1) charge X_aligned_phase1.csv et Y_aligned_phase1.csv
# 2) garde un identifiant sample propre
# 3) enlève les colonnes non numériques
# 4) calcule sparsité / variance
# 5) filtre les colonnes trop rares
# 6) applique log1p sur Y
# 7) sauvegarde des matrices propres pour la suite
# ============================================================
...
def presence_fraction(df: pd.DataFrame) -> pd.Series:
    """
    Fraction de samples où la colonne est > 0
    """
    if df.empty:
...
def main():
    ...
    output_dir = repo_root / "10_analysis" / "outputs" / "phase2_preprocessing"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print("PHASE 2 - PREPARE ML MATRICES")
    print("=" * 70)
    ...
    # On garde un sample_id unique de référence
    ...
    print(f"Same order between aligned X and Y: {same_order}")
    print()
    ...
    # ------------------------------------------------------------
    # Présence (>0) par colonne
    # ------------------------------------------------------------
    x_presence = presence_fraction(X_num)
    y_presence = presence_fraction(Y_num)

    # seuils simples et explicites
    # X: garder colonnes présentes dans au moins 5% des samples
    # Y: garder colonnes présentes dans au moins 10% des samples
    x_min_presence = 0.05
    y_min_presence = 0.10
    ...
    # ------------------------------------------------------------
    # Enlever colonnes de variance nulle
    # ------------------------------------------------------------
    x_var = X_filt.var()
    y_var = Y_filt.var()
    ...
    # colonnes supprimées par filtre de présence
